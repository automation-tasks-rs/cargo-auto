...
pub fn auto_delete_old_js_snippets() -> Result<()> {
    let current_dir = std::env::current_dir().log()?;
    ...
    for entry in std::fs::read_dir(snippets_dir).log()? {
        let entry = entry?;
        let second_folder = entry.path();
        let second_name = entry
            .file_name()
            .into_string()
            .map_err(|_| Error::ErrorFromStr("file_name into_string error")).log()?
            .to_lowercase();
        if !second_name.starts_with("dodrio") {
            //println!("{:?}",second_folder);
            let second_metadata = std::fs::metadata(&second_folder).log()?;
            ...
            match opt_first_mtime {
                ...
                Some(first_mtime) => match second_mtime.cmp(&first_mtime) {
                    ...
                    std::cmp::Ordering::Greater => {
                        let first_folder = opt_first_folder.ok_or_else(|| Error::ErrorFromStr("opt_first_folder is None")).log()?;
                        println!("  {YELLOW}delete first: {:?}{RESET}", first_folder);
                        std::fs::remove_dir_all(first_folder).log()?;
                    ...
                    std::cmp::Ordering::Less => {
                        ...
                        std::fs::remove_dir_all(second_folder).log()?;
                    }
                    // else
