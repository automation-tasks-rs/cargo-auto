...
pub fn task_build() -> anyhow::Result<cl::CargoToml> {
    ...
    cl::run_shell_command_static("cargo clippy --no-deps --target x86_64-pc-windows-gnu").log()?;
    cl::run_shell_command_static("cargo build --target x86_64-pc-windows-gnu").log()?;
    Ok(cargo_toml)
}
...
pub fn task_release() -> anyhow::Result<cl::CargoToml> {
    ...
    cl::run_shell_command_static("cargo fmt").log()?;
    cl::run_shell_command_static("cargo clippy --no-deps --target x86_64-pc-windows-gnu").log()?;
    cl::run_shell_command_static("cargo build --release --target x86_64-pc-windows-gnu").log()?;
