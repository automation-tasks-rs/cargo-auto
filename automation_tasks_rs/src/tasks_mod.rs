...
pub fn task_doc() -> anyhow::Result<()> {
    ...
    if cargo_toml.package_name() != "cargo_auto_lib" {
        cl::auto_plantuml(&cargo_toml.package_repository().context("repository is None").log()?).log()?;
        cl::auto_playground_run_code().log()?;
    }
    cl::auto_md_to_doc_comments().log()?;

    cl::run_shell_command_static("cargo doc --no-deps --document-private-items").log()?;
    // copy target/doc into docs/ because it is GitHub standard
    cl::run_shell_command_static("rsync -a --info=progress2 --delete-after target/doc/ docs/").log()?;
...
pub fn task_commit_and_push(arg_2: Option<String>) -> anyhow::Result<()> {
    ...
    if !cl::git_is_local_repository().log()? {
        cl::new_local_repository(&message).context("new_local_repository is None").log()?;
    }

    // If needed, ask to create a GitHub remote repository
    if !cgl::git_has_remote().log()? || !cgl::git_has_upstream().log()? {
        cgl::new_remote_github_repository().log()?;
        cgl::description_and_topics_to_github().log()?;
    } else {
        // if description or topics/keywords/tags have changed
        cgl::description_and_topics_to_github().log()?;

        // separate commit for docs if they changed, to not make a lot of noise in the real commit
        if CrossPathBuf::new("docs").log()?.exists() {
            cl::run_shell_command_static(r#"git add docs && git diff --staged --quiet || git commit -m "update docs" "#).log()?;
        }

        cl::add_message_to_unreleased(&message).log()?;
        ...
        cl::run_shell_command_static("git push").log()?;
    }
    Ok(())
}
...
pub fn task_github_new_release() -> anyhow::Result<()> {
    ...
    // take care of tags
    let tag_name_version = cl::git_tag_sync_check_create_push(&version).log()?;

    let github_owner = cargo_toml.github_owner().context("repository is None").log()?;
    ...
    .log()?;
    let json_value = ende::github_api_token_with_oauth2_mod::send_to_github_api_with_secret_token(request).log()?;
    // early exit on error
    ...
    if std::fs::exists(&executable_path).log()? {
        ...
        // upload asset
        cgl::github_api_upload_asset_to_release(&github_owner, &repo_name, &release_id, &format!("tmp/{compressed_name}")).log()?;
