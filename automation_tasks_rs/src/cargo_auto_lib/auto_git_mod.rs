...
pub fn git_is_local_repository() -> Result<bool> {
    let output = std::process::Command::new("git").arg("status").output().log()?;
    let output = String::from_utf8(output.stderr).log()?;
    // return bool
...
pub fn process_git_remote() -> String {
    ...
    fn git_remote_output() -> Result<String> {
        let output = std::process::Command::new("git").arg("remote").arg("-v").output().log()?;
    ...
    fn regex_capture(output: String) -> Result<String> {
        let reg = regex::Regex::new(r#"origin\s*(?:https://).log()?(?:git@).log()?([^:/]*?)[:/]([^/]*?)/([^. ]*?)(?:\.git).log()?\s*\(fetch\)"#).log()?;
        let cap = reg.captures(&output).ok_or(Error::ErrorFromStr("Error: reg.captures is None")).log()?;
...
pub fn new_local_repository(message: &str) -> Result<()> {
    ...
    let answer = inquire::Text::new(&format!("{BLUE}Do you want to initialize a new local git repository? (y/n){RESET}")).prompt().log()?;
    ...
    if !camino::Utf8Path::new("docs").exists() {
        std::fs::create_dir("docs").log()?;
        std::fs::write("docs/index.html", "project docs").log()?;
    }

    // create new local git repository and commit all on branch main
    crate::cargo_auto_lib::run_shell_command_static("git config --global init.defaultBranch main").log()?;
    crate::cargo_auto_lib::run_shell_command_static("git init").log()?;
    crate::cargo_auto_lib::run_shell_command_static("git add .").log()?;
    crate::cargo_auto_lib::run_shell_command(&format!(r#"git commit -m "{message}""#)).log()?;
    crate::cargo_auto_lib::run_shell_command_static("git branch -M main").log()?;
    Ok(())
}
