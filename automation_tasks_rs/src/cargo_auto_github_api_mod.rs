...
pub(crate) fn git_has_upstream() -> anyhow::Result<bool> {
    ...
    let output = std::process::Command::new("git").arg("branch").arg("-vv").output().log(pos!())?;
    // return
    Ok(String::from_utf8(output.stdout).log(pos!())?.contains("["))
}
...
pub(crate) fn new_remote_github_repository() -> anyhow::Result<()> {
    ...
    let json_value = send_to_github_api_with_secret_token(github_api_get_authenticated_user()).log(pos!())?;
    ...
    let authenticated_user_login = authenticated_user_login
        .as_str()
        .context("authenticated_user_login is None")
        .log(pos!())?;
    ...
    if github_owner_or_organization == authenticated_user_login {
        ...
        let json_value = send_to_github_api_with_secret_token(github_api_get_organization(&github_owner_or_organization)).log(pos!())?;
        ...
        let json_value = if github_owner_or_organization == authenticated_user_login {
            ...
            if let Some(error_message) = json_value.get("message") {
                ...
                if let Some(errors) = json_value.get("errors") {
                    let errors = errors.as_array().context("errors as_array is None").log(pos!())?;
    ...
    if !git_has_upstream().log(pos!())? {
        cl::run_shell_command("git push -u origin main").log(pos!())?;
...
pub(crate) fn description_and_topics_to_github() -> anyhow::Result<()> {
    ...
    let github_owner_or_organization = cargo_toml.github_owner().context("github_owner is None").log(pos!())?;
    let description = cargo_toml
        .package_description()
        .context("package_description is None")
        .log(pos!())?;
    ...
    if is_old_metadata_different {
        ...
        let json =
            send_to_github_api_with_secret_token(github_api_get_repository(&github_owner_or_organization, &repo_name)).log(pos!())?;
        ...
        if !topics_is_equal {
            ...
            std::fs::write(
                ...
                serde_json::to_string_pretty(&old_metadata).log(pos!())?,
            )
            .log(pos!())?;
        }
    }
...
pub(crate) fn github_api_upload_asset_to_release(
    ...
    let file = CrossPathBuf::new(path_to_file).log(pos!())?;
    let file_name = file.to_path_buf_current_os();
    let file_name = file_name.file_name().context("file_name is None").log(pos!())?.to_string_lossy();
    ...
    let mut release_upload_url = <url::Url as std::str::FromStr>::from_str(&release_upload_url).log(pos!())?;
    release_upload_url.set_query(Some(format!("{}={}", "name", file_name).as_str()));
    let file_size = std::fs::metadata(file.to_path_buf_current_os()).log(pos!())?.len();
    println!("  {YELLOW}It can take some time to upload. File size: {file_size}. Wait...{RESET}");
    // region: async code made sync locally
    let rt = tokio::runtime::Runtime::new().log(pos!())?;
...
pub(crate) fn github_api_create_new_release(
    ...
    */
    ...
    let body = serde_json::to_string_pretty(&body).log(pos!())?;
